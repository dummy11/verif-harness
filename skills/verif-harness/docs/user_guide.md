...
八份内置目录只是候选文档目录；实际纳入本轮的文档数量记为 N，N 可以少于、等于或多于八。
默认对每份在范围内的文档建立一个公开的文档撰写方案节点和一个公开的文档交付节点，因此默认是
N 份文档、N 个撰写方案节点、N 个交付节点。若一份文档确实需要不同负责人或不同 gate，才显式拆成
多个公开节点。Dashboard 的文档目录还会展示正文版本、内容是否变化、开放问题、工程决定和评审记录。
VDOC 对外只有两种节点类型：`document-writing-plan`（文档撰写方案）和
`document-deliverable`（文档交付）。两者都必须用 `document_key` 明确归属一份文档。方案节点描述准备怎样写，交付节点描述已经写出的正文内容，二者不能复用同一内容模板。目标与范围、计划写入内容、待确认工程决定、输入依据、
交付对象及依赖影响都是撰写方案节点内部可独立审批的区块，不再拆成其他节点类型。
Engine 在公开节点内部建立负责人不可见且带稳定摘要的工作子节点，用于定位章节、审批变更项和依赖影响。
这些子节点与其他工作节点使用相同的状态、依赖、Activity、Agent assignment、问题、支持材料、失效传播和
closure 机制，但不是负责人待办，不能单独形成负责人审批结论；公开节点仍是审批与状态聚合边界。
...
Human 审批撰写方案节点。全部方案通过后，Agent 才完成正文、同步摘要并登记纯交付 proposal，Human
...
当前 VDOC 只有同时满足以下条件才算收敛：所有必需文档撰写方案节点均已确认审批完成；所有必需
文档交付节点均针对当前节点定义和当前正文批准，且相应审批已经由 Main Agent 检查；一份文档的全部
必需正文内容已由其交付节点覆盖；Human 确认项、Agent questions、修改/说明请求、阻塞问题和开放工程决定均已关闭；没有暂定、过期、内容
...
Dashboard 的 VDOC 工作流主页面只保留“当前项目状态”和“工作节点列表”。工作流级操作只有
“添加节点”“删除节点”和“重新启动工作流”；方案审批、正文验收、问题确认和支持材料判断都在
对应节点中完成，不在主页面再建立一套整条工作流审批。添加和删除只登记负责人对下一版方案的
变更要求，不会直接创建或删除当前节点；“删除”表示从下一版方案移出，历史记录仍然保留。

“重新启动工作流”需要点击后再次确认，并填写操作人和原因。确认后 Engine 创建新的 VDOC
revision，停止当前版本的活动和待处理评审，使旧节点审批不再计入新版本，并回到 Agent 形成
文档撰写方案的阶段；新的方案再次批准前不会出现正文内容验收节点。该操作不会删除或覆盖已有
验证文档、历史节点、审批、验收或支持材料，也不会重启 Dashboard、CLI 或整个验证项目。规范
事实源仍是上面的 VDOC 文档；页面状态不能替代版本化规则或负责人的工程判断。
...
| 14. 登记正文评审 | Human + Engine | Human 先选择审批分类；“要求修改”时可逐项登记新增、修改和删除要求，再填写审批内容并提交。Engine 将结论绑定到节点定义摘要、文件 SHA-256 和版本，并把节点置为“Agent 正在检查审批”，不会立即显示验收通过 |
| 15. Agent 检查审批 | Agent + Engine | Main Agent 读取待检查审批，核对当前正文和依赖影响，自行判断是否需要 Human 确认。需要时登记绑定该节点的 Agent question；不需要或所有问题解决并重新分析后，登记检查完成 |
| 16. 聚合验收状态 | Engine | 只有当前正文已有 Human 审批通过、对应 Agent 检查已经完成且该节点全部 Agent questions 已解决，节点才变为“已验收通过” |
| 17. 列出未完成项 | Engine + Agent | Engine 通过 `closure` 列出当前还缺的最小动作；Agent 向 Human 解释，不静默执行写操作 |
| 18. 冻结 VDOC | Human + Agent + Engine | Human 明确同意冻结；Agent 调用 `freeze VDOC`；Engine 检查文件指纹，并保存已评审正文和文件清单的快照 |
| 19. 后续修订 | Agent + Engine + Human | Agent 修改受影响正文后调用 `docs sync`；Engine 把相关结论标为需要重新验证；Human 重新评审 |
...
1. **规划审批**：Dashboard 对每个 DUT-specific VDOC 方案节点整体审批；全部必需方案完成审批后才允许
...
在独立项目之间切换；切换后所有读取、事件流和负责人写操作都重新绑定到所选项目自己的
`.verif-harness/model.sqlite3`。首页的工作流卡片以三列排列（窄屏自动变为两列或一列），圆弧仅表示当前
方案版本中已有完成结论的工作节点比例，不表示 Agent 运行时间或验证质量。“需要你处理”、“Agent 交互”
和“风险与变更”移到侧边栏；首页不再重复放置待处理、支持材料或分页标签。项目与验证对象默认折叠。
方案编制提示、工作流和工作节点的状态灯、进度环与进度条支持缓慢呼吸显示：执行中为蓝色
（约 2.6 秒），等待执行、审批、回答或处理为黄色（约 3.4 秒）。动画只强调已登记状态，
不表示 Agent 心跳、在线检测或新增进度；百分比和进度条长度仍取自当前版本的真实记录。
0% 时只让轨道周围的光晕呼吸，不填入虚假进度。完成、失败、失效和取消等状态保持静态；
系统启用“减少动态效果”（`prefers-reduced-motion`）时全部呼吸动画关闭，保留颜色、状态文字和数值。
节点详情与列表使用相同的进度计算和呼吸规则；没有正式方案时仅提示方案编制，不创建工作流或节点。
从审计记录取得的负责人 reviewer 和 Agent actor、
...
Dashboard 默认在当前标签页内导航；正常点击不会额外打开标签页。工作节点先在右侧抽屉
快速查看，可选“查看完整节点”。文档撰写方案节点只显示本节点的撰写方案、可展开的节点审批、
...
验证文档仍以 `.md` 为唯一可编辑正文；Agent 和工程师编辑 Markdown，Dashboard 将同一份
正文动态渲染为 HTML 阅读视图，不保存第二份 HTML 文档。只读正文页、正文验收页和文档评审
弹窗均默认展示排版后的标题、列表、引用、表格、链接和代码块，底部可展开“查看 Markdown
源码”。渲染器随 Dashboard 本地提供，不依赖外部 CDN；加载失败时明确提示并显示源码。
文档中的原始 HTML 按文字显示，Mermaid 等代码块按代码显示；图片目前显示描述，不自动
访问外部图片或本机路径。相对 `.md` 链接只打开当前项目已登记且存在的文档，保留项目和授权
上下文；缺失目标显示为不可用文字并提示原因。章节链接使用标题生成的锚点，跨文档章节
不存在时会提示并打开整篇正文。外部 HTTP/HTTPS 链接新开标签页且不携带 Dashboard 来源信息。
审批仍绑定 Markdown 的摘要与正文版本；未同步内容会提示负责人等待 Agent 同步和重新核对。

浏览器前进、后退保留站内导航；返回列表时保留本标签页中的搜索、节点展开状态和滚动位置。
关闭抽屉后刷新不会再次打开已关闭节点。实时快照刷新保留同一项目、节点和版本的未提交
表单输入、结构化修改要求和输入焦点。草稿只存于当前标签页内存，不是审批记录；刷新或关闭
标签页前，浏览器会提示可能丢失未提交输入。主动新开的标签页不复制草稿。
版本或内容摘要变化时，旧草稿只作为可复制的只读参考展示，不自动填入新版本或提交。
跨项目不继承草稿；提交成功后清除对应表单草稿，其他未提交表单仍保留。

Dashboard 入口清单（所有内部导航、前进后退和新标签页均保留当前项目与授权 token）：

| 入口 | 目标与状态来源 | 失效时的处理 |
| --- | --- | --- |
| 首页标志、侧栏“项目概览” | 当前项目的首页快照 | 项目未注册时提示并提供已注册项目入口 |
| 顶部项目选择器、注销项目 | 已注册项目列表、项目注销确认与 API | 项目不存在或无权访问时显示错误，不切到相似项目 |
| 侧栏“需要你处理” | 当前项目待办汇总，来自同一快照；每项显示操作类型、对应工作节点、所属文档和本次具体审批/验收/确认内容，点击后直接打开该工作节点，不建立另一套待办详情页 | 项目不可用时显示错误 |
| 侧栏“Agent 交互” | 当前项目 Agent 问答页，来自同一快照 | 项目不可用时显示错误 |
| 侧栏“风险与变更” | 当前项目风险页，来自同一快照 | 项目不可用时显示错误 |
| 侧栏工作流、首页工作流卡片 | 当前标签页的对应工作流页，两种入口行为一致 | 工作流不存在时提示并可返回概览 |
...
界面入口回归覆盖侧栏和卡片点击、抽屉/完整页面、前进后退、刷新和带参数直达，以及项目/
工作流/节点/问题失效情况；写操作还须以 API 与重新读取的快照核对。真实浏览器回归位于
`tests/test_dashboard_navigation.py`，需要 Node.js、Playwright 和浏览器；本机 Chrome 可设置
`VERIF_DASHBOARD_BROWSER_CHANNEL=chrome`。无 Playwright 时跳过该可选浏览器测试，不能因此声称浏览器验证通过。
...
2. **验证工作流**：显示每条 Workstream 的工作节点进度和状态；用缓慢呼吸的进度圈表达可量化完成度，
   点击后在当前标签页打开；需要对照时主动新开标签页；节点列表为了横向比较仍使用进度条；
3. **需要你处理**：从侧栏进入待办汇总，按对象归并当前需要负责人审批、验收或确认的事项；每项必须
   明确显示对应工作节点、所属文档和本次具体处理内容，不能只重复通用动作名称；
   “Agent 交互”同样从侧栏进入，不在首页重复一套面板；
4. **辅助信息**：项目与验证对象默认折叠；风险与变更只作为次要入口显示数量。

首页不重复显示“等待负责人处理”的横幅、状态卡和工作流计数。工作流或工作节点本身表达等待状态，
待办汇总只列出实际可处理的事项。正式方案审批、文档交付验收和文档工程决定按对象归并；Agent questions
通过侧栏“Agent 交互”入口处理。负责人已经提交、等待 Agent 执行的修改请求不重复算作
负责人待办。
“需要你处理”页面沿用首页的单张状态卡层级，只在同一张卡片中列出待办，不再追加项目上下文、问题区、
审批区或其他重复板块。它是工作节点入口汇总，不是独立工作流；点击待办后直接打开对应工作节点。
...
方案审批页和文档验收页直接说明审批或验收的对象、内容、结论边界和下一步。
内部编号、文档路径、规则版本和内容摘要只放在“查看内部信息”中。需要负责人操作的入口仍统一放在
“需要你处理”，各详情页只提供与当前对象直接相关的审批、验收、评论或确认操作。

“需要你处理”页按类型归并审批、验收和确认待办；Agent question 统一从“Agent 交互”进入。该页面只展示
主 Agent 当前等待负责人回答的问题、对应回答框，以及已回答问题的历史交互记录；不展示主 Agent
运行详情、子 Agent 工作状态、普通 Activity 或审批/验收待办。这样“等待你处理”明确表示当前存在开放的
Agent question；没有开放问题时显示“无需你处理”。工作节点列表出现“Agent 等待你回答”时，点击这段
状态文字会打开“Agent 交互”页并定位到该节点的第一个未回答问题；点击节点名称仍然打开节点详情。
...
外部开放问题会进入侧栏“需要你处理”；点击后在当前标签页打开对应文档节点的抽屉。
...
| 工作流 | 工作节点应当具体到 |
| --- | --- |
| VDOC | 默认每份在范围内的文档对应一个 `document-writing-plan` 和一个 `document-deliverable`；章节、语义范围和依赖影响映射为负责人不可见、但具备完整工作节点能力的内部子节点。只有确有独立负责人或独立 gate 时才拆成多个公开节点。交付节点必须同时具备内部子节点完成、当前版本的负责人审批通过、Agent 检查完成和零个未解决 Agent questions |
...
负责人提交文档交付节点的验收结论后，Engine 创建持久化 Agent 检查点。提交并不直接让节点变成
“已验收通过”。Main Agent 必须读取待检查项，检查审批内容、当前正文版本和依赖影响，并自行判断
是否需要继续向负责人提问：

```text
verif-harness agent-review-check list --status PENDING
verif-harness agent-question ask NODE --prompt TEXT \
  --option ID LABEL DESCRIPTION --option ID LABEL DESCRIPTION --no-wait
verif-harness agent-review-check complete REVIEW_ID \
  --summary "已检查当前审批、正文和依赖影响；没有未解决问题"
```

如果 Agent 提问，检查点进入等待负责人回答；答案写回后重新变为待 Agent 检查，Main Agent 必须重新
分析，不能因为问题状态已变成 `ANSWERED` 就自动完成。正文验收节点只有同时满足“负责人对当前版本
提交审批通过”和“Main Agent 检查已完成且该节点所有 Agent questions 已解决”才显示“已验收通过”。
