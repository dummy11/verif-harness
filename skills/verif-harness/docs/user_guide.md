...
Dashboard 入口清单（所有入口均绑定当前选中项目；打开新标签页的入口同时携带写入 token）：
...
| 侧栏工作流、首页工作流卡片 | 对应当前项目的工作流页 | 工作流不存在时提示并可返回概览 |
| 工作节点与问题入口 | 当前工作流的节点详情或 Agent 问答 | 节点或工作流不在当前版本时提示并可返回概览 |
| 方案审批、正文验收、文档查看 | 当前版本节点及受控文档 API | 对象不存在或已过期时拒绝操作并提示 |
| 工作流重启 | 当前 VDOC 工作流的二次确认与受控 API | 当前版本不匹配时拒绝操作 |

界面入口回归覆盖侧栏点击、刷新和带参数直达，以及项目/工作流/节点失效情况；写操作还须以 API 与重新读取的快照核对。
...
3. **需要你处理**：右侧唯一的行动区域，按对象归并当前需要负责人审批、验收或确认的事项；每项必须
   明确显示对应工作节点、所属文档和本次具体处理内容，不能只重复通用动作名称；底部
   “Agent 交互”只显示“等待你处理”或“无需你处理”等状态；
4. **辅助信息**：项目与验证对象默认折叠；风险与变更只作为次要入口显示数量。

首页不重复显示“等待负责人处理”的横幅、状态卡和工作流计数。工作流或工作节点本身表达等待状态，
右侧只列出实际可处理的事项。正式方案审批、文档交付验收和文档工程决定按对象归并；Agent questions
通过同一区域中的“Agent 交互”状态入口处理。负责人已经提交、等待 Agent 执行的修改请求不重复算作
负责人待办。
...
首页右侧按类型归并审批、验收和确认待办；Agent question 统一从“Agent 交互”进入。该页面只展示
