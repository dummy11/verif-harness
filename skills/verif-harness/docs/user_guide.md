...
Engine 在公开节点内部维护负责人不可见且带稳定摘要的语义单元，用于定位章节、审批变更项和依赖影响；这些
内部单元不是负责人待办，不能单独形成通过结论，公开节点仍是审批与状态聚合边界。
...
| 工作流 | 工作节点应当具体到 |
| --- | --- |
| VDOC | 默认每份在范围内的文档对应一个 `document-writing-plan` 和一个 `document-deliverable`；章节、语义范围和依赖影响作为负责人不可见的内部细粒度单元。只有确有独立负责人或独立 gate 时才拆成多个公开节点。交付节点必须同时具备当前版本的负责人审批通过、Agent 检查完成和零个未解决 Agent questions |
