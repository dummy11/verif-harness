...
当前 VDOC 只有同时满足以下条件才算收敛：所有必需文档撰写方案节点的必需区块均已批准且已确认审批完成；所有必需
...
| 7. 逐节点审批实施方案 | Human | 负责人在 Dashboard 当前标签页的节点审批页面逐项检查由当前 DUT 分解出的 VDOC 方案节点；“待确认工程问题”“计划写入内容”“输入依据、范围和交付对象”分别作出批准、修改、澄清或拒绝决定；存在实际依赖时还需单独审批“依赖和影响” |
...
快速查看，可选“查看完整节点”。文档撰写方案节点只显示本节点的撰写方案、可展开的逐项审批、
...
分别审批各部分内容；全部必需内容批准后，可用旁边的“审批完成”改变该工作节点状态，且之后仍能
...
VDOC 在每个按当前 DUT 分解出的文档撰写方案节点内直接展开审批；待确认工程问题、计划写入内容、
输入依据、范围、交付对象以及实际存在的依赖影响分别审批。逐项批准只授权开始工作，不表示文档正文、
实现或证据已经通过验收；“审批完成”只更新该撰写方案工作节点状态，不会关闭审批入口。
