...
**Dashboard（实时项目看板）**是只在本机打开的 Web 页面。每个系统账号自动选择的 loopback
端口上可以注册该账号的多个验证项目；同机不同账号使用不同服务和端口，不会互相复用。顶部选择器
只决定当前请求访问哪个项目；项目之间不共享 SQLite、Workstream、节点、
...
不同工作域的节点正文不能只是更换名称。VENV 节点描述接口、时钟复位域、环境组件或观测路径；
VSTIM 描述具体激励功能和场景；VCHK 描述检查目标、结果检查模块、参考结果路径或断言；VCOV 描述
具体覆盖目标；VCASE 描述功能与用例的对应关系或具体用例；VREG 描述回归配置、批次和失败分组。
VDOC 比较特殊：一份正式文档作为一个顶层文档节点是合适的，但该节点只是文档容器。文档中的开放
问题、工程决定和评审记录作为节点内事项展示，正文仍保存在 Markdown 中；系统不会为每个段落建节点。
