...
Use the Verification Reasoning Engine only when schemas and deterministic tools cannot resolve intent
conflicts, uncertain impact, triage hypotheses, or alternative closure
strategies.

Run `reason capabilities`, then prepare `reason ROLE PURPOSE [--backend
BACKEND]`; automation may use the expanded `reason request --role ROLE
--purpose PURPOSE` form. Role and Backend are independent. A backend response must contain
