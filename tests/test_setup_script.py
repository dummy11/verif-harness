...
class SetupScriptTest(unittest.TestCase):
    ...
    def test_script_installs_all_managed_surfaces_and_launches_selected_cli(self) -> None:
        ...
        self.assertIn('kimi_help="$("$agent_cli" --help 2>&1)"', source)
        self.assertIn("lacks --prompt/--continue support", source)
        self.assertIn('kimi_mcp_config="$workspace_root/.kimi-code/mcp.json"', source)
        self.assertIn("Kimi project MCP config is missing before inventory", source)
        self.assertIn('kimi_inventory_args=(--prompt "$startup_inventory_prompt")', source)
        self.assertIn('if [[ "$kimi_help" == *"--mcp-config-file"* ]]', source)
        self.assertIn('kimi_inventory_args=(--mcp-config-file "$kimi_mcp_config"', source)
        self.assertIn("using project MCP auto-discovery", source)
        self.assertIn('if "$agent_cli" "${kimi_inventory_args[@]}"', source)
