...
class SetupScriptTest(unittest.TestCase):
    ...
    def test_script_installs_all_managed_surfaces_and_launches_selected_cli(self) -> None:
        ...
        self.assertIn("lacks --mcp-config-file/--prompt/--continue support", source)
        ...
        self.assertIn('if "$agent_cli" --mcp-config-file "$kimi_mcp_config"', source)
        self.assertIn('--prompt "$startup_inventory_prompt"', source)
        self.assertNotIn('"${agent_args[@]}" --mcp-config-file', source)
